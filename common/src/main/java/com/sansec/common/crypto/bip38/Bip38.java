package com.sansec.common.crypto.bip38;
...
//    private static final int SCRYPT_P = 1;
    ...
    private static final BigInteger n = new BigInteger(1, Base64Tools.HexUtils.fromHex("fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141"));
    ...
    public static String encryptNoEcMultiply(CharSequence passphrase, String base58EncodedPrivateKey) throws ValidationException, InterruptedException {
        byte[] tmp = Base64Tools.Base58.decode(base58EncodedPrivateKey);
    ...
    public static String encryptNoEcMultiply(byte[] stretcedKeyMaterial, BitCoinECKeyPair key, byte[] salt) {
        ...
        // Checksum
        byte[] checkSum = Base64Tools.SHA256.doubleSha256(encoded, 0, 39);
        ...
        // Base58 encode
        return Base64Tools.Base58.encode(encoded);
    }
    ...
    public static Bip38PrivateKey parseBip38PrivateKey(String bip38PrivateKey) {
        // Decode Base 58
        byte[] decoded = Base64Tools.Base58.decode(bip38PrivateKey);
        if (decoded == null) {
            return null;
        }
    ...
    public static BitCoinECKeyPair decryptEcMultiply(Bip38PrivateKey bip38Key, CharSequence passphrase
        ...
        if (bip38Key.lotSequence) {
            ...
            passFactor = Base64Tools.SHA256.doubleSha256(tmp);
        }
...
//        byte[] derived = SCrypt.generate(passPoint, saltPlusOwnerSalt, 1024, 1, 1, 64);
        ...
        // Generate factorB
        byte[] factorB = Base64Tools.SHA256.doubleSha256(seedB);
    ...
    public static byte[] calculateScryptSalt(String address) {
        byte[] hash = Base64Tools.SHA256.doubleSha256(address.getBytes());
        byte[] ret = new byte[4];
