...
public class BitCoinECKeyPair extends ECKeyPair {
    ...
    public static BitCoinECKeyPair parseWIF(String wif) throws ValidationException {
        byte[] decode = Base64Tools.Base58.decode(wif);

        //判断私钥格式
        byte[] check = new byte[4];
        System.arraycopy(decode, decode.length - 4, check, 0, check.length);
        byte[] privateCheck = Base64Tools.SHA256.doubleSha256(decode, 0, decode.length - 4);
        for (int i = 0; i < 4; i++) {
    ...
    public String getPrivateKey() {
        ...
        byte[] check = Base64Tools.SHA256.doubleSha256(rawPrivateKey, 0, rawPrivateKey.length - 4);
        System.arraycopy(check, 0, rawPrivateKey, rawPrivateKey.length - 4, 4);
        return Base64Tools.Base58.encode(rawPrivateKey);
    }

    @Override
    public String getPublicKey() {
        return Base64Tools.HexUtils.toHex(getRawPublicKey());
    }

    @Override
    public String getAddress() {
        return Base64Tools.Base58.encode(getRawAddress());
    }
    ...
    public byte[] getRawAddress() {
        ...
        if (isCompressed()) {
            hashedPublicKey = Base64Tools.RIPEMD160.hash160(pubComp);
        } else {
            hashedPublicKey = Base64Tools.RIPEMD160.hash160(pub);
        }
        ...
        //进行双 Sha256 运算
        byte[] check = Base64Tools.SHA256.doubleSha256(addressBytes, 0, addressBytes.length - 4);
    ...
    private static void checkChecksum(byte[] store) throws ValidationException {
        ...
        byte[] hash = Base64Tools.SHA256.doubleSha256(ekey);
        for (int i = 0; i < 4; ++i) {
    ...
    private static BTCTransaction signatureForm(BTCTransaction transaction, int index, String
        ...
        for (int i = 0; i < transaction.inputs.length; i++) {
            ...
            if (i == index) {
                //设置地址签名
                script = new BTCTransaction.Script(Base64Tools.HexUtils.fromHex(publicScript));
            }
    ...
    private static String mkPubKeyScript(String address) {
        ...
        //设置签名
        byte[] bytes = Base64Tools.Base58Check.base58ToBytes(address);
        byte[] checkByte = new byte[bytes.length - 1];
        System.arraycopy(bytes, 1, checkByte, 0, checkByte.length);
        stringBuilder.append(Base64Tools.HexUtils.toHex(checkByte));
