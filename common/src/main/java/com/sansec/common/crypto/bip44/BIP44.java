...
import com.sansec.common.exception.CoinNotFindException;
import com.sansec.common.exception.NotSupportException;
...
public final class BIP44 {
    ...
    public static AddressIndex parsePath(String path) throws NotSupportException, CoinNotFindException {
        // m/44'/60'/0'/0/0
        ...
        // 字符串是否与正则表达式相匹配
        if (!matcher.matches()) {
            throw new NotSupportException("Path format is not correct");
        }
        ...
        if ((Integer.parseInt(purposeStr)) != 44) {
            throw new NotSupportException("Only support the 44 protocol");
        }
